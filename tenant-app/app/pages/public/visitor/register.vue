    ...
    <div class="w-full flex-1 flex flex-col relative min-h-[400px]">
        ...
        <div v-if="step === 1" class="space-y-8 animate-fade-in">
            ...
            <a-button type="primary" block size="large"
                class="h-12 text-lg font-medium rounded-xl shadow-lg shadow-blue-500/20" :loading="loading"
                @click="sendOtp">
        ...
        <div v-else-if="step === 2" class="space-y-8 text-center animate-fade-in pt-12">
            ...
            <div class="flex justify-center gap-4 py-4">
                <input v-for="i in 4" :key="i" type="tel" inputmode="numeric" pattern="[0-9]*" maxlength="1"
                    ...
                    @keydown.delete="e => handleOtpBackspace(e, i - 1)" ref="otpInputs" />
            </div>
            ...
            <button
                ...
                :disabled="loading" @click="verifyOtpCode">
                <span v-if="loading">
                    <LoadingOutlined class="!text-white" />
                </span>
                <span v-else class="!text-white">Verify
                    <CheckCircleFilled class="ml-1 !text-white" />
                </span>
            </button>
        </div>
        ...
        <div v-else-if="step === 3" class="space-y-6 animate-fade-in pt-12">
            ...
            <a-button type="primary" block size="large"
                class="h-12 text-lg font-medium rounded-xl shadow-lg shadow-blue-500/20" @click="step = 4">
                Continue &rarr;
            </a-button>
        ...
        <div v-else-if="step === 4" class="animate-fade-in">
            <HostSelection v-model="formState.selectedHost" @confirm="handleFinish"
                @manual-entry="message.info('Manual entry not implemented')"
                @change-company="message.info('Change company not implemented')" />
        </div>

        <!-- Camera Modal -->
        <a-modal v-model:open="showCamera" :footer="null" :closable="false" width="100%"
            wrap-class-name="full-screen-modal" destroy-on-close>
        ...
        <a-modal v-model:open="showCountryModal" :footer="null" title="Select Country" centered :width="400"
            wrap-class-name="public-light-modal">
            <div class="p-2">
...
const sendOtp = async () => {
    ...
    setTimeout(() => {
        loading.value = false
        step.value = 2
        startResendTimer() // Start the resend cooldown when OTP is first sent
