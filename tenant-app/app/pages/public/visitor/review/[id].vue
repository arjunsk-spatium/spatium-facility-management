<template>
    <div v-if="visitor" class="max-w-md mx-auto min-h-screen bg-gray-50 flex flex-col font-sans">
        ...
        <div class="flex-1 p-4 pb-24 overflow-y-auto space-y-4">
            ...
            <div
                class="bg-white rounded-3xl p-6 text-center shadow-sm border border-orange-100 relative overflow-hidden">
                <!-- Background Decoration -->
                <div
                    class="absolute top-0 left-0 w-full h-24 bg-gradient-to-b from-orange-50 to-transparent opacity-50">
                </div>

                <div class="mb-4 relative z-10">
                    <div
                        class="w-24 h-24 rounded-full mx-auto border-4 border-white shadow-sm overflow-hidden bg-orange-100 mb-3">
                        <img :src="visitor.photo_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${visitor.name}`"
                            class="w-full h-full object-cover" />
                    </div>
                    <span
                        class="bg-white border border-gray-100 px-2 py-0.5 rounded-md text-[10px] font-bold text-blue-600 shadow-sm">
                ...
                <div class="grid grid-cols-2 gap-3 mt-6">
                    <a :href="`mailto:${visitor.email}`"
                        class="h-10 rounded-xl bg-gray-50 border border-gray-100 text-gray-700 font-bold text-xs flex items-center justify-center gap-2 hover:bg-gray-100">
                        <MailOutlined /> Email
                    </a>
            ...
            <div class="bg-white rounded-2xl p-4 shadow-sm border border-gray-50 flex gap-4 items-start">
                <div
                    class="w-10 h-10 rounded-xl bg-purple-50 flex items-center justify-center text-purple-600 shrink-0">
                    <FileTextFilled />
                </div>
                <div>
        ...
        <div v-if="visitor.status === 'Pending'"
            class="fixed bottom-0 left-0 w-full bg-white p-4 border-t border-gray-100 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] flex gap-4 z-20">
